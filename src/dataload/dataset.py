...
class TrainGraphDataset(TrainDataset):
    ...
    def line_mapper(self, line, sum_num_news, tokenizer, conti_tokens):
        ...
        if self.cfg.model.use_entity:
            origin_entity = candidate_input[:, -3 - self.cfg.model.entity_size:-3]  #[5, 5]     此处截取原文中实体的embedding，正样本有一个，负样本有四个，所以合起来有五个候选新闻，第一个表示正样本
            ...
            candidate_entity = np.concatenate((origin_entity, candidate_neighbor_entity), axis=-1)    # 将邻居实体和自身实体连接起来，准备下一步的操作
        else:
    ...
    def build_prompt(self,click_id, pos, neg, tokenizer, conti_tokens):
        ...
        for i, news in enumerate(click_id):
            ...
            his_news_num.append(i)          # 用数字表示表示浏览历史，占位，以便后面将embedding进行替换
            # hcate=cate+" "+subcate
    ...
    def build_subgraph(self, subset, k, sum_num_nodes):
        ...
        unique_subset, unique_mapping = torch.unique(subset, sorted=True, return_inverse=True)    # 获取 subset 中的唯一值，并返回两个张量。unique_subset 包含了唯一的节点索引，而 unique_mapping 包含了将原始 subset 中的值映射到 unique_subset 中的索引。
